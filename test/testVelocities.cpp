...
int main(int argc, char**argv){
    ...
    if (!(f=fopen(readPath, "r"))){
        ...
        Point p1;
            ...
            data.clear();
            f = fopen(filePath, "a");
            fprintf(f,"%.3f,   %.3f pi\t\t%.3f,   %.3f pi\t\t%.3f,    %.3f pi\t\t%.3f,   %.3f pi\n", r.vector.p.Length(), atan(r.vector.p.y/r.vector.p.x)/M_PI, r.affineResult.p.Length(), 
            atan(r.affineResult.p.y/r.affineResult.p.x)/M_PI, r_w.vector.p.Length(), atan(r_w.vector.p.y/r_w.vector.p.x)/M_PI, r_w.affineResult.p.Length(), 
            atan(r_w.affineResult.p.y/r_w.affineResult.p.x)/M_PI);
            fclose(f);
