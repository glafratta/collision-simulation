...
void step( AlphaBot &motors){
   ...
   if (confIteration != c->getIteration()){ //every odd iteration because data comes in at even iteration and it needs time for processing
        c->applyController();
        motors.setRightWheelSpeed(-(c->getCurrentState()->getAction().leftWheelSpeed)); //temporary fix because motors on despacito are the wrong way around
        motors.setLeftWheelSpeed(-(c->getCurrentState()->getAction().rightWheelSpeed));
