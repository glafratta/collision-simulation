...
int main(int argc, char** argv){
    ...
    float x=conf.transitionSystem[v0].endPose.p.x;
    float y=conf.transitionSystem[v1_old].endPose.p.y;
    b2Vec2 d_pos(x, y);
    sr.collision= Disturbance(AVOID, d_pos);
    sr.collision.bodyFeatures().setHalfWidth(0.05);
    sr.collision.bodyFeatures().setHalfWidth(0.05);
    sr.step=24;
    ...
    gt::set(e.first, sk, conf.transitionSystem, v1==conf.currentVertex, conf.errorMap, 2);
