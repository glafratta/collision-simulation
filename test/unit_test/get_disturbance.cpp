...
int main(int argc, char** argv){
    ...
    vertexDescriptor v, src;
    Direction src_d=DEFAULT, curr_d=DEFAULT;
    if (argc>2){
        src_d=Direction(atoi(argv[2]));
    } 
    conf.transitionSystem[conf.currentVertex].options.push_back(src_d);
    ...
    Disturbance result = conf.getDisturbance(conf.transitionSystem, v, &world, curr_d);
   // std::pair <edgeDescriptor, bool> edge_src=conf.add_vertex_now(conf.currentVertex, src, conf.transitionSystem, obstacle);  
    conf.transitionSystem[src].options.push_back(src_d);
    if (argc>3){
    ...
    Disturbance result = conf.getDisturbance(conf.transitionSystem, v, &world, conf.transitionSystem[edge.first].direction);
    if (result==solution){
        return 0;
    }
    return 1;
