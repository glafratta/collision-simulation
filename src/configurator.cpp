	...
	float theta;
		...
		if (!transformMatrix.empty()){
			...
			if (tmp.Length()>maxAbsSpeed){
				...
				tmp.x = state->getTrajectory().getLinearSpeed() * state->getMaxSpeed()*cos(tmpAngle);
				tmp.y = state->getTrajectory().getLinearSpeed() * state->getMaxSpeed()*(tmpAngle);
